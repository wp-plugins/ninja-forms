<div id="post-elements" class="postbox" name="field-settings-list" >
	<div class="handlediv" id="handle-post-settings" title="Click to toggle"><br></div>
		<h3 >
			<span><?php _e('Post Elements', 'ninja-forms');?></span>
		</h3>
		<div class="inside" id="post-settings" <?php if($post_settings_state == 'closed' OR $echoed){ echo 'style="display:none;"'; }else{$echoed = true;}?>>
			<p class="howto"><?php _e('If you want NinjaForms to create a post from user input, check the box below and then insert Post-specific fields', 'ninja-forms');?>.</p>
			<p class="button-controls">
				<label for="ninja_form_post">
					<input type="hidden" name="ninja_post" value="unchecked"><input type="checkbox" id="ninja_form_post" name="ninja_post" value="checked" <?php echo $ninja_post;?>> <?php _e('Create Post From Input', 'ninja-forms');?>
				</label>
			...
			<div class="button-controls ninja_post_options" id="ninja_post_options" style="<?php if($ninja_post != "checked"){ echo "display:none;";}?>">
				<p class="button-controls">
					<label for="post_options[login]">
						<input type="hidden" name="post_options[login]" value="unchecked"><input type="checkbox" name="post_options[login]" id="post_options[login]" value="checked" <?php echo $post_options['login'];?>> <?php _e('Users Must Be Logged In To Post', 'ninja-forms');?>
					</label>	
					<?php if($plugin_settings['admin_help'] == 'checked'){ ?><img id="require_login" src="<?php echo NINJA_FORMS_URL;?>/images/question-ico.gif"><?php } ?>
				</p>
				<p class="button-controls">
					<?php _e('Users post as', 'ninja-forms');?>
					<?php
				...
				<p class="button-controls">
					<?php _e('Select a post type', 'ninja-forms');?>: 
				...
				<p class="button-controls">
					<?php _e('Select a post status', 'ninja-forms');?>:
					<select id="" name="post_options[post_status]">
						<option value="draft" <?php if($post_options['post_status'] == 'draft'){ echo 'selected';}?>><?php _e('Draft', 'ninja-forms');?></option>
						<option value="pending" <?php if($post_options['post_status'] == 'pending'){ echo 'selected';}?>><?php _e('Pending', 'ninja-forms');?></option>
						<option value="publish" <?php if($post_options['post_status'] == 'publish'){ echo 'selected';}?>><?php _e('Publish', 'ninja-forms');?></option>
					</select>
				...
				<p class="button-controls">
					<span <?php if($new_posttitle == 'no'){ ?>style="display:none;"<?php } ?> id="ninja_new_posttitle_<?php echo $ninja_forms_row['id'];?>_cont">
						<a class="button add-new-h2 ninja_new_field" id="ninja_new_posttitle_<?php echo $ninja_forms_row['id'];?>" href="#"><?php _e('Post Title', 'ninja-forms');?></a>
					</span>
				</p>			
				<p class="button-controls">
					<span <?php if($new_postcontent == 'no'){ ?>style="display:none;"<?php } ?> id="ninja_new_postcontent_<?php echo $ninja_forms_row['id'];?>_cont">
						<a class="button add-new-h2 ninja_new_field" id="ninja_new_postcontent_<?php echo $ninja_forms_row['id'];?>" href="#"><?php _e('Post Content', 'ninja-forms');?></a>
				...
				<p class="button-controls">
					<span <?php if($new_postexcerpt == 'no'){ ?>style="display:none;"<?php } ?> id="ninja_new_postexcerpt_<?php echo $ninja_forms_row['id'];?>_cont">
						<a class="button add-new-h2 ninja_new_field" id="ninja_new_postexcerpt_<?php echo $ninja_forms_row['id'];?>" href="#"><?php _e('Post Excerpt', 'ninja-forms');?></a>
					</span>
				</p>			
				<p class="button-controls">
					<span <?php if($new_postcat == 'no'){ ?>style="display:none;"<?php } ?> id="ninja_new_postcat_<?php echo $ninja_forms_row['id'];?>_cont">
						<a class="button add-new-h2 ninja_new_field" id="ninja_new_postcat_<?php echo $ninja_forms_row['id'];?>" href="#"><?php _e('Post Category', 'ninja-forms');?></a>
				...
				<p class="button-controls">
					<span <?php if($new_posttags == 'no'){ ?>style="display:none;"<?php } ?> id="ninja_new_posttags_<?php echo $ninja_forms_row['id'];?>_cont">
						<a class="button add-new-h2 ninja_new_field" id="ninja_new_posttags_<?php echo $ninja_forms_row['id'];?>" href="#"><?php _e('Post Tags', 'ninja-forms');?></a>
					</span>
				</p>
		</div>
	</div>
