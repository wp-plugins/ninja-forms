...
class NF_Subs_CPT {
		...
		if ( isset ( $_GET['form_id'] ) ) {
			...
			if ( $column == 'id' ) {
				...
				if ( !isset ( $_GET['post_status'] ) || $_GET['post_status'] == 'all' ) {
					...
					echo '<span class="edit"><a href="post.php?post=' . $sub_id . '&action=edit&ref=' . urlencode( esc_url( add_query_arg( array() ) ) ) . '" title="' . __( 'Edit this item', 'ninja-forms' ) . '">Edit</a> | </span> 
			...
			<script type="text/javascript">
				jQuery(document).ready(function() {
					...
					if ( isset ( $_REQUEST['download_all'] ) && $_REQUEST['download_all'] != '' ) {
						...
						$redirect = esc_url_raw( remove_query_arg( array( 'download_all' ), $redirect ) );
						?>
	...
	public function jquery_remove_counts() {
		...
		if ( $typenow == 'nf_sub' && $pagenow == 'edit.php' ) {
			...
			$all_url = esc_url( remove_query_arg( 's', $all_url ) );
			$trash_url = add_query_arg( array( 'post_status' => 'trash' ) );
			$trash_url = esc_url( remove_query_arg( 's', $trash_url ) );
			if ( isset ( $_GET['form_id'] ) ) {
