...
class NF_Notifications
{
	...
	public function output_admin() {
		...
		if ( '' == $action ) {
			?>
			<h2><?php _e( 'Notifications', 'ninja-forms' ); ?> <a href="<?php echo esc_url( add_query_arg( array( 'notification-action' => 'new' ) ) ); ?>" class="add-new-h2"><?php _e( 'Add New', 'ninja-forms' );?></a></h2>
	...
	public function save_admin( $form_id, $data ) {
		...
		if ( $new ) {
			...
			$redirect = esc_url_raw( add_query_arg( array( 'id' => $n_id, 'notification-action' => 'edit' ), $redirect ) );
