jQuery(document).ready(function($) {
	...
	$(".ninja_multi_form_next").click(function(event){
		...
		var ninja_submit = 1;
		$("#ninja_multi_page_" + current_page).find('.ninja_req').each(function(){
		...
		if(ninja_submit == 1 || $("#ninja_next_req").val() == 'unchecked'){
			...
			$("#ninja_form_overlay").attr("innerHTML", ajax.ninja_forms_required + "<br /> <a href='#' class='ninja_close_dialog'>Close</a>");
	...
	function ninjaSubmit(formData, jqForm, options){
		...
		if(ninja_submit == 0){
			...
			$("#ninja_form_overlay").attr("innerHTML", ajax.ninja_forms_wait + "<br />");
			return true;
		}
	}
	...
	$("#ninja_show_continue_login").click(function(event){
		event.preventDefault();
			...
			if(data == 'fail'){
				...
				$.each(obj.fields, function(i,data){
					...
					if(data.type == 'textbox' || data.list_type == 'select' || data.type == 'posttitle' || data.type == 'posttags'){
						...
						$('[name="ninja_field_' + data.id + '"][value="' + value +'"]').attr('checked',true);
						...
						$("[name='ninja_field_" + data.id + "']").append("<div>" + ajax.ninja_forms_uploaded_file + ": " + data.value + "</div>");
					}
				});
