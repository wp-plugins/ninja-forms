jQuery(document).ready(function($) {
	...
	$(".ninja_multi_form_next").click(function(event){
		...
		$("#ninja_multi_name").prop("innerHTML", new_name);
		if($("#ninja_multi_progress").val() == 'checked'){
			var multi_count = $("#ninja_multi_count").val();
		...
		$("#ninja_multi_step").prop("innerHTML", next_page);
	...
	$(".ninja_multi_form_previous").click(function(event){
		...
		$("#ninja_multi_step").prop("innerHTML", previous_page);
		goToByScroll('ninja_form_top');	
	});	
	...
	function ninjaSubmit(formData, jqForm, options){
		...
		if(ninja_submit == 0){
			$("#ninja_form_overlay").prop("innerHTML", "Please ensure that all required fields are filled out properly. <br /> <a href='#' class='ninja_close_dialog'>Close</a>");
				
			$("#ninja_form").show();
			return false;
		}else{
			$("#ninja_form_overlay").prop("innerHTML", "Please Wait...<br />");
			return true;
		}
	}
	function ninjaResponse(responseText, statusText, xhr, $form){
		...
		if(tmp == 'pass'){
			...
			if(ninja_ajax_submit != 'checked'){
				...
				$("#ninja_form").prop("innerHTML", responseText);
				$("#ninja_form_overlay").dialog('close');
			}
		}else{
			$("#ninja_form_overlay").prop("innerHTML", tmp + "<br /> <a href='#' class='ninja_close_dialog'>Close</a>");
			$("#ninja_form_overlay").dialog('open');
		}
	}
				...
				$.each(obj.fields, function(i,data){
					...
					if(data.type == 'textbox' || data.type == 'posttitle' || data.type == 'posttags'){
						...
						if(data.value == 'checked'){
							$("#ninja_field_" + data.id).prop('checked', true);
						}
						...
						$("#ninja_field_" + data.id + "[value=" + value + "]").prop("checked", true);
					}else if(data.list_type == 'multi' || data.type == 'postcat'){
						var selected = value.split("|ninja|");
						for(var i in selected){
							$("#ninja_field_" + data.id + " option[value=" + selected[i] + "]").prop('selected', true);
						}
